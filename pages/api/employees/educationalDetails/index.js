...
function getStatuses(req, res) {
    ...
    if (search !== '' && searchbyfield !== '') {
        queryv = `SELECT count(*) as numRows FROM educationaldetails WHERE status=1 AND ${searchbyfield} LIKE '%${search}%'`;
    } else if (search !== '' && searchbyfield === '') {
        queryv = `SELECT count(*) as numRows FROM educationaldetails WHERE status=1 AND user_id LIKE '%${search}%'`;
    } else {
        queryv = 'SELECT count(*) as numRows FROM educationaldetails WHERE status=1';
    }

    connection.query(queryv, function (err, rows, fields) {
        try {
            if (err) { res.status(200).json({ message: "Connection Query Error", status: 0 }); }
            else {
                ...
                let q;
                if (search !== '' && searchbyfield !== '') {
                    q = `SELECT * FROM educationaldetails WHERE ${searchbyfield} LIKE '%${search}%' AND status=1 ORDER BY ${sortByfield} ${sort} LIMIT ${limit} OFFSET ${skip}`;
                }
                else if (search !== '' && searchbyfield == '') {
                    q = `SELECT * FROM educationaldetails WHERE user_id LIKE '%${search}%' AND status=1 ORDER BY ${sortByfield} ${sort} LIMIT ${limit} OFFSET ${skip}`;
                }
                else {
                    q = `SELECT * FROM educationaldetails WHERE status=1 ORDER BY ${sortByfield} ${sort} LIMIT ${limit} OFFSET ${skip}`;
                }
