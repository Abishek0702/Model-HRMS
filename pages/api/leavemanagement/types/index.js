...
function getUsers(req, res) {
		...
		connection.query('SELECT count(*) as numRows FROM leavestatus',function (err, rows, fields) {
			...
			if(err) {
				...
				if (search != "") {
					var q='SELECT * FROM leavetypes where TypeofLeave  like "%'+search+'%" AND status =1 order by id desc LIMIT ' + limit; 
				}else{
				var q='SELECT * FROM leavetypes order by id desc LIMIT ' + limit; 
				}
			//	console.log("q: ", q);
